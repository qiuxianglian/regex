...
public class 正则 {
    ...
    public boolean 匹配节点(抽象节点 节点,流 节点流,String 新字符串,AtomicInteger 字符串指针,boolean 更新指针){
        if(节点 == null) return true;
        if(节点.为标志符节点()){
            if(!匹配标志符节点(节点.转为标志符节点(),节点流,新字符串,字符串指针,更新指针)){

                return false;
            }
        }else if(节点.为匹配星()){
            if(!匹配星节点(节点.转为匹配星(),节点流,新字符串,字符串指针,更新指针)){

                return false;
            }
        }else if(节点.为匹配站位()){
            if(!匹配站位节点(节点.转为匹配站位(),节点流,新字符串,字符串指针,更新指针)){
    ...
    public boolean 匹配标志符节点(标志符节点 标志符节点,流 节点流,String 新字符串,AtomicInteger 字符串指针,boolean 更新指针){
        int 备份指针 = 字符串指针.get();
    ...
    public boolean 匹配星节点(匹配星 匹配星节点,流 节点流,String 新字符串,AtomicInteger 字符串指针,boolean 更新指针){
    ...
    public boolean 匹配站位节点(匹配站位 匹配站位节点,流 节点流,String 新字符串,AtomicInteger 字符串指针,boolean 更新指针){
        if(字符串指针.get()>=新字符串.length()){
            return false;
        }
        if(更新指针){
